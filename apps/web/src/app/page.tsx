...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      {/* Hero */}
      <section className="bg-panel px-5 pb-7 pt-8 text-on-dark">
        <span className="inline-block border border-dark-line-2 px-2 py-1 text-2xs font-semibold tracking-wider text-on-dark-2">
          {t.eyebrow}
        </span>
        <h1 className="mt-5 font-display text-7xl font-bold tracking-tighter">{t.heroTitle}</h1>
        <p className="mt-4 max-w-[300px] text-md leading-relaxed text-on-dark-3">{t.heroBody}</p>
        <Link
          href="/register"
        ...
        {lang === 'en' && (
          <p className="mt-4 text-xs text-on-dark-5">{t.langNote}</p>
        )}
      </section>
      ...
      <section className="px-5 pt-8">
        ...
        <div className="mt-3">
          {steps.map(([title, body], index) => (
            <div key={title} className="flex gap-3.5 border-t border-line py-3.5">
      ...
      {/* Footer */}
      <footer className="mt-8 bg-panel px-5 pb-8 pt-7 text-on-dark">
